...
function getClassStatus(now) {
  ...
  const end = 12 * 60 + 30;
  if (current < start)
    return { label: "Class starts soon", color: "yellow", active: false };
  if (current >= start && current < end)
    return { label: "Class in progress", color: "green", active: true };
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            <span
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border ${csColors[classStatus.color]}`}
            >
              <span
                className={`w-2 h-2 rounded-full ${classStatus.active ? "bg-green-400 animate-pulse" : classStatus.color === "yellow" ? "bg-yellow-400" : "bg-gray-400"}`}
              />
              {classStatus.label}
            </span>
            <span
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div
          className={`col-span-2 rounded-xl border p-4 ${dark ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}
        >
          <h3
            className={`text-sm font-semibold mb-3 ${dark ? "text-gray-400" : "text-gray-500"}`}
          >
            Weekly Attendance
          </h3>
          {weeklyData.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-[220px]">
              <p
                className={`text-sm ${dark ? "text-gray-600" : "text-gray-400"}`}
              >
                No data
              </p>
            </div>
      ...
      {liveFeed.length > 0 && (
        <div
          className={`rounded-xl border p-4 mb-6 ${dark ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}
        >
          <h3
            className={`text-sm font-semibold mb-3 flex items-center gap-2 ${dark ? "text-gray-400" : "text-gray-500"}`}
          >
          ...
          <div className="space-y-2">
            {liveFeed.map((item, i) => (
              <div
                key={i}
                className={`flex items-center gap-3 text-sm px-3 py-2 rounded-lg ${dark ? "bg-gray-800/50" : "bg-gray-50"}`}
              >
                <span
                  className={`w-2 h-2 rounded-full ${item.type === "checkin" ? "bg-green-500" : "bg-blue-500"}`}
                />
                <span
                ...
                <span className={dark ? "text-gray-500" : "text-gray-400"}>
                  {item.type === "checkin" ? "checked in" : "checked out"} at{" "}
                  {item.time}
                </span>
                {item.status === "late" && (
